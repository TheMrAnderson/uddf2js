const { parseUDDF } = require('../src/uddf');
const fs = require('fs');
...
describe('parseUDDF', () => {
	test('handles subsurface manually entered data', async () => {
		const xmlData = loadTestXml('subsurface_manual.uddf');
		const result = await parseUDDF(xmlData, 'imperial');
	...
	test('returns SI units by default', async () => {
		const xmlData = loadTestXml('sample1.uddf');
		const result = await parseUDDF(xmlData);
	...
	test('converts to metric units', async () => {
		const xmlData = loadTestXml('sample1.uddf');
		const result = await parseUDDF(xmlData, 'metric');
	...
	test('converts to imperial units', async () => {
		const xmlData = loadTestXml('sample1.uddf');
		const result = await parseUDDF(xmlData, 'imperial');
		expect(result.unit).toBe('imperial');
		const waypoints = result.data.uddf.profiledata.repetitiongroup[0].dive[0].samples.waypoint;
	...
	test('handles multiple waypoints', async () => {
		const xmlData = loadTestXml('sample1.uddf');
		const result = await parseUDDF(xmlData, 'metric');
		expect(result.unit).toBe('metric');
		const waypoints = result.data.uddf.profiledata.repetitiongroup[0].dive[0].samples.waypoint;
	...
	test('handles dive computer', async () => {
